...
function displayForecast(forecastData) {
  ...
  Object.values(days)
    ...
    .forEach((item) => {
      ...
      const iconPath =
        weatherIconMap[item.weather[0].main] || weatherIconMap["Clear"];
      ...
      <div class="forecast-date">${new Date(item.dt * 1000).toLocaleDateString(
        undefined,
        { weekday: "short" }
      )}</div>
    `;
...
function checkWeatherAlerts(data) {
  ...
  if (temp > 35) {
    showWeatherAlert(
      "Extreme heat warning! Temperature above 35°C. Stay hydrated and avoid outdoor activities."
    );
  } else if (temp < 0) {
    showWeatherAlert(
      "Freezing temperature alert! Roads may be icy. Drive carefully."
    );
  }
  ...
  if (condition === "thunderstorm") {
    showWeatherAlert(
      "Thunderstorm warning! Seek shelter indoors and avoid outdoor activities."
    );
  } else if (condition === "snow") {
    showWeatherAlert("Snow alert! Expect travel delays and slippery roads.");
  } else if (condition === "rain" && data.wind.speed > 10) {
    showWeatherAlert(
      "Heavy rain and strong winds! Use caution when traveling."
    );
  }
}
